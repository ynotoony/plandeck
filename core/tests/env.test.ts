...
describe("env 扫描", () => {
  it("发现 *_API_KEY 类变量并归入清单（只保留指纹）", async () => {
    const plans = await scanEnvPlans({
    ...
    expect(plans.map((p) => [p.name, p.source, p.sourceDetail, p.hasCredential])).toEqual([
      ["CLAUDE", "env", "CLAUDE_AUTH_TOKEN", true],
      ["DEEPSEEK", "env", "DEEPSEEK_APIKEY", true],
      ["GEMINI", "env", "GEMINI_ACCESS_TOKEN", true],
      ["MINIMAX", "env", MINIMAX_ENV_VAR, true],
      ["OPENAI", "env", "OPENAI_API_KEY", true],
    ]);
  ...
  it("同一 base 的多个变量（API_KEY / APIKEY）都保留，id 追加序号", async () => {
    const plans = await scanEnvPlans({
      FOO_API_KEY: "a",
      FOO_APIKEY: "b",
    });
    expect(plans.map((p) => p.id)).toEqual(["env-foo", "env-foo-2"]);
    expect(plans.every((p) => p.credentialFingerprint && !("key" in p))).toBe(true);
  });

  it("空 env 返回空清单", async () => {
    await expect(scanEnvPlans({})).resolves.toEqual([]);
  });
});
